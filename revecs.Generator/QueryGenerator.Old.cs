...
public class QueryGenerator
{
    ...
    private Dictionary<string, HashSet<string>> _queryPerFile = new();
...
{{
    ...
    /// <summary>Whether or not the query is optional for execution checks</summary>
    internal interface {OptionalType} {{ }}
...
";
    ...
    {
        Log(3, "Found query");
        Log(4, "IsRecord: " + source.IsRecord);
        var sb = new StringBuilder();
        sb.Append(@"using revecs;
...
");
        ...
        var isSingleton = source.Option.Any(t => t.Name == SingletonType);

        void BeginNamespace()
        {
            if (source.Parent?.ContainingNamespace is { } ns)
        ...
        void CreateArguments()
        {
            ...
            foreach (var arg in source.Option)
            {
                var queryArgument = new QueryArgument
                {
                    IsOption = true,
...
");
        ...
        var additionalInterfaceSb = new StringBuilder();
...
");
            ...
            additionalInterfaceSb.Append($@"
        ArchetypeQueryEnumerator GetEnumerator() => throw new NotImplementedException();
");
        }
        ...
        {
            additionalInterfaceSb.Append($@"
        UEntityHandle Entity => throw new NotImplementedException();
");                
            sb.AppendLine(@"
                    ...
                    foreach (var f in arg.Symbol.GetMembers())
                    {
                        ...
                        switch (f)
                        {
                            case IFieldSymbol fieldSymbol:
                                ...
                                additionalInterfaceSb.Append($@"
        ref{ro} {fieldSymbol.Type.GetTypeName()} {fieldSymbol.Name} => throw new NotImplementedException();");
...
");
                            ...
                            case IPropertySymbol propertySymbol:
                                ...
                                additionalInterfaceSb.Append($@"
        ref{ro} {propertySymbol.Type.GetTypeName()} {propertySymbol.Name} 
        {{ 
            {(propertySymbol.GetMethod == null ? "" : "get;")} 
            {(propertySymbol.SetMethod == null ? "" : "set;")} 
        }}");
...
");
            ...
            void MultipleArg()
            {
                foreach (var arg in arguments)
                {
                    ...
                    {
                        additionalInterfaceSb.Append($@"
        {access} {arg.Symbol.GetTypeName()} {arg.Symbol.Name} => throw new NotImplementedException();");
        ...
        {{ 
            ...
            {{
                 return ref (Query.Any() 
                    ...
                    {
                        additionalInterfaceSb.Append($@"
        {access} {viaWorld.ValueType} {arg.Symbol.Name} => throw new NotImplementedException();");
...
");
            ...
            additionalInterfaceSb.Append($@"
        {source.Name}.Enumerator GetEnumerator() => throw new NotImplementedException();
");
...
{iterationInit}
            ...
            public Enumerator({source.GetDisplayName()} self)
            {{
                this = default;
...
");
        ...
        void CreateInterface()
        {
            ...
            interfaceSb.Append($"    private interface IQuery");
            if (validArgs.Any())
            {
                interfaceSb.Append('<');
                interfaceSb.Append(
            ...
            foreach (var arg in arguments)
            {
                ...
                var constraint = arg.CanWrite switch
                {
                    true => $"Write<{arg.Symbol.GetTypeName()}>",
                    false => $"Read<{arg.Symbol.GetTypeName()}>"
                };

                interfaceSb.Append("            "); // indent
                interfaceSb.AppendLine($"where T{arg.Symbol.Name} : {constraint}");
            }

            interfaceSb.AppendLine("    {");
            interfaceSb.AppendLine(additionalInterfaceSb.ToString());
            interfaceSb.Append("        ");
            interfaceSb.AppendLine(
                $"bool Any() => throw new NotImplementedException();");
            interfaceSb.Append("        ");
            interfaceSb.AppendLine(
                $"int GetEntityCount() => throw new NotImplementedException();");
            interfaceSb.AppendLine("    }");

            var str = interfaceSb.ToString();
            ref var set = ref CollectionsMarshal.GetValueRefOrAddDefault(_queryPerFile, source.FilePath, out _);
                ...
                .OfType<TypeDeclarationSyntax>())
            {
                ...
                if (queryInterface != null)
                {
                    ...
                    (
                        ...
                        symbol.Name,
                        tree.FilePath,
                        ImmutableArrayExtensions.Select(queryInterface.TypeArguments, t =>
                        {
                            Log(3, $"Found Argument {t.Name}");
                            return (INamedTypeSymbol) t;
                        }).ToArray(),
                        symbol.Interfaces.Where(inter =>
                        {
